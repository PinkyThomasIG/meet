...
export const getEvents = async (numEvents = 32) => {
  ...
  if (token) {
    removeQuery();
    const url =
      "https://wnt211j7yj.execute-api.eu-central-1.amazonaws.com/dev/api/get-events" +
      "/" +
      token;
...
export const getAccessToken = async () => {
  ...
  if (!accessToken || tokenCheck.error) {
    ...
    if (!code) {
      const response = await fetch(
        "https://wnt211j7yj.execute-api.eu-central-1.amazonaws.com/dev/api/get-auth-url"
      ); // serveless get auth url
      const result = await response.json();
...
const getToken = async (code) => {
  ...
  const response = await fetch(
    "https://wnt211j7yj.execute-api.eu-central-1.amazonaws.com/dev/api/token" +
      "/" +
